...
/* Rekap nilai (Evaluasi, KB1, KB2) sekarang dikirim ke endpoint Flask ini
   (/api/rekap), BUKAN langsung ke URL Google Apps Script dari browser.
   app.py yang meneruskan ke Apps Script di sisi server (lihat GOOGLE_SHEETS_URL
   di app.py / .env) — ini menghindari batasan CORS browser dan, yang lebih
   penting, membuat pesan sukses/gagal yang ditampilkan ke siswa jadi BENAR
   (sebelumnya `fetch(..., {mode:'no-cors'})` membuat browser tidak pernah
   bisa tahu apakah pengiriman ke Spreadsheet benar-benar berhasil). */
...
/**
 * Kirim payload rekap (tipe 'evaluasi' atau 'progresKB') ke backend Flask
 * sendiri (/api/rekap), yang lalu meneruskannya ke Google Apps Script di
 * sisi server. Berbeda dari versi lama (fetch langsung ke Apps Script
 * dengan mode:'no-cors'), fungsi ini benar-benar MEMBACA balasannya —
 * kalau Apps Script gagal (deployment nonaktif, error skrip, dll), error
 * itu di-throw supaya bisa ditampilkan ke pengguna, bukan disembunyikan
 * di balik pesan "berhasil" yang keliru.
 */
async function kirimKeRekap(payload) {
  ...
  try {
    ...
    // fetch() sendiri gagal (offline / server tidak terjangkau sama sekali) —
    // beda dari server membalas dengan status error.
    throw new Error('Tidak ada koneksi ke server (cek internet kamu).');
  }
  const data = await res.json().catch(() => ({}));
  if (!res.ok || data.status !== 'ok') {
    if (res.status === 404) {
      // 404 di sini berarti request TIDAK PERNAH sampai ke handler
      // /api/rekap di app.py (bukan Apps Script/Spreadsheet yang error) —
      // paling sering karena hosting yang dipakai tidak benar-benar
      // menjalankan app.py (mis. cuma hosting statis untuk HTML/CSS/JS),
      // atau server Flask belum di-restart setelah deploy terbaru.
      throw new Error(
        'Server backend (app.py) tidak ditemukan di hosting ini (HTTP 404). ' +
        'Hasil disimpan sementara di perangkat ini dan akan dicoba kirim ulang otomatis.'
      );
    }
