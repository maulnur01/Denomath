...
/* ──────────────────────────────────────────────
   ...
   ke OpenAI/Anthropic API dari browser.

   Kenapa harus lewat backend?
   - Memanggil API AI langsung dari browser akan
   ...
   - Lewat backend, API key disimpan aman di server
     (file .env), TIDAK PERNAH dikirim ke browser siswa.

   ⚠️ PENTING — CARA MENJALANKAN (ringkas, lihat app.py
   untuk panduan lengkap termasuk versi XAMPP):
   1. Buka terminal, masuk ke folder project ini.
   2. Install dependency Python (sekali saja):
        pip install -r requirements.txt
   3. Salin file .env.example menjadi .env, lalu isi
      SALAH SATU: OPENAI_API_KEY (ChatGPT) atau
      ANTHROPIC_API_KEY (Claude) dengan API key kamu.
   4. Jalankan backend:
        python app.py
   ...
   5. Buka index.html di browser (boleh double-click
      file-nya, lewat XAMPP/Apache, atau lewat server
      statis seperti `python -m http.server 8000`).
   6. Jika backend BELUM dijalankan / API key belum diisi,
      widget "Tanya AI" akan menampilkan pesan yang jelas
...
────────────────────────────────────────────── */
/* ──────────────────────────────────────────────
   DENO AI CONFIG — Groq API (langsung dari browser)
   Model: llama-3.3-70b-versatile (GRATIS & sangat cepat)
   Daftar & ambil API key di: https://console.groq.com/keys
   Tidak perlu backend Python/Flask.
────────────────────────────────────────────── */
const GROQ_API_URL = 'https://api.groq.com/openai/v1/chat/completions';
const GROQ_MODEL   = 'llama-3.3-70b-versatile';
let   GROQ_API_KEY = localStorage.getItem('denomath_groq_key') || '';
...
async function cekStatusBackendAI() {
  ...
  if (GROQ_API_KEY) {
    setStatusAI('Siap membantu! (Groq ⚡)');
  } else {
    setStatusAI('API Key belum diatur');
    tambahBubbleAI(
      'assistant',
      '🔑 <strong>DENO AI membutuhkan Groq API Key</strong> untuk berfungsi.<br><br>' +
      'Klik tombol <strong>⚙️ Atur API Key</strong> di bawah, masukkan API Key Groq kamu, lalu simpan.',
      { isError: false, trusted: true }
    );
    setTimeout(() => tampilkanPanelApiKey(), 400);
  }
}

/** Tampilkan panel input API Key Groq */
function tampilkanPanelApiKey() {
  const existing = document.getElementById('aiApiKeyPanel');
  if (existing) { existing.remove(); return; }

  const panel = document.createElement('div');
  panel.id = 'aiApiKeyPanel';
  panel.className = 'ai-apikey-panel';
  panel.innerHTML = `
    <div class="ai-apikey-header">
      <span>🔑 Atur Groq API Key</span>
      <button onclick="document.getElementById('aiApiKeyPanel').remove()" style="background:none;border:none;color:var(--text-3);cursor:pointer;font-size:1.1rem">✕</button>
    </div>
    <p style="font-size:0.78rem;color:var(--text-3);margin-bottom:10px;line-height:1.5">Masukkan Groq API Key kamu. Tersimpan hanya di browser ini (tidak dikirim ke server manapun selain Groq).</p>
    <input type="password" id="groqKeyInput" class="ai-apikey-input" placeholder="gsk_..." value="${GROQ_API_KEY}">
    <div style="display:flex;gap:8px;margin-top:10px">
      <button class="ai-apikey-btn-save" onclick="simpanGroqKey()">💾 Simpan & Aktifkan</button>
      <button class="ai-apikey-btn-clear" onclick="hapusGroqKey()">🗑️ Hapus Key</button>
    </div>
    <div style="margin-top:10px;font-size:0.75rem;color:var(--text-3);line-height:1.5">
      Belum punya API Key? → <a href="https://console.groq.com/keys" target="_blank" style="color:var(--biru-langit)">Groq Console (GRATIS)</a>
    </div>
  `;
...
function simpanGroqKey() {
  const input = document.getElementById('groqKeyInput');
  if (!input) return;
  const key = input.value.trim();
  if (!key) { showToast('⚠️ API Key tidak boleh kosong!', 'warn'); return; }
  GROQ_API_KEY = key;
  localStorage.setItem('denomath_groq_key', key);
  document.getElementById('aiApiKeyPanel')?.remove();
  setStatusAI('Siap membantu! (Groq ⚡)');
  showToast('✅ API Key Groq tersimpan!', 'success');
  tambahBubbleAI('assistant', '✅ API Key berhasil disimpan! Sekarang kamu bisa mulai bertanya tentang Redenominasi Rupiah 💰📊');
}

function hapusGroqKey() {
  GROQ_API_KEY = '';
  localStorage.removeItem('denomath_groq_key');
  document.getElementById('aiApiKeyPanel')?.remove();
  setStatusAI('API Key belum diatur');
  showToast('🗑️ API Key dihapus', 'warn');
}
...
/** Kirim pertanyaan ke Groq API langsung dari browser */
async function sendAIMessage() {
  ...
  // Cek API key dulu
  if (!GROQ_API_KEY) {
    tampilkanPanelApiKey();
    showToast('⚠️ Masukkan Groq API Key terlebih dahulu!', 'warn');
    return;
  }
  ...
  const systemPrompt = `Kamu adalah DENO AI, asisten belajar ramah di dalam aplikasi DENOMATH - media pembelajaran interaktif untuk siswa SMP (Kelas 7-9) di Indonesia, yang membahas materi "Redenominasi Rupiah" dan "Penyajian Data" (tabel, diagram batang, diagram lingkaran).

ATURAN UTAMA:
1. Gunakan Bahasa Indonesia yang ramah, sederhana, dan menyemangati - seperti kakak pembimbing. Boleh sesekali pakai emoji relevan (💰📊🧮).
2. Fokus topik: redenominasi rupiah (penyederhanaan angka mata uang dengan dibagi 1.000, nilai daya beli TETAP SAMA, beda dengan sanering), serta penyajian data (tabel dan diagram batang/lingkaran).
3. Rumus inti: Harga/Nilai Baru = Harga/Nilai Lama / 1.000.
4. JANGAN langsung memberikan jawaban akhir dari soal evaluasi. Bantu dengan menjelaskan cara berpikir/rumus/langkah penyelesaiannya.
5. Jawaban ringkas dan jelas: idealnya 2-5 kalimat atau beberapa poin singkat.
6. Jangan pernah berpura-pura menjadi manusia.`;

  // Bangun messages dalam format OpenAI-compatible (Groq pakai standar ini)
  const messages = [{ role: 'system', content: systemPrompt }];
  const riwayatBersih = state.aiHistory.slice(-20, -1)
    .filter(item => (item.role === 'user' || item.role === 'assistant') && item.content?.trim());
  riwayatBersih.forEach(item => messages.push({ role: item.role, content: item.content.trim() }));
  ...
  try {
    ...
    if (!response.ok) {
      let pesanError = 'Maaf, DENO AI sedang tidak bisa diakses.';
      if (response.status === 401) pesanError = '⚠️ API Key tidak valid. Cek kembali Groq API Key kamu!';
      else if (response.status === 403) pesanError = '⚠️ API Key tidak punya akses. Periksa izin key di console.groq.com.';
      else if (response.status === 429) pesanError = '⚠️ Batas permintaan tercapai. Tunggu sebentar lalu coba lagi.';
      else if (data.error?.message) pesanError = `⚠️ Error: ${data.error.message}`;
    ...
    if (!replyText) {
      tambahBubbleAI('assistant', '⚠️ Maaf, aku tidak mendapat respons dari AI. Coba lagi ya!', { isError: true });
      setStatusAI('Sedang bermasalah');
      return;
    }

    tambahBubbleAI('assistant', replyText);
    state.aiHistory.push({ role: 'assistant', content: replyText });
    setStatusAI('Siap membantu! (Groq)');
    bicarakanAI(replyText);
    ...
    tambahBubbleAI(
      'assistant',
      '⚠️ Tidak dapat terhubung ke Groq API.<br><br>' +
      'Pastikan kamu terhubung ke internet dan Groq API Key sudah dimasukkan dengan benar.',
