...
────────────────────────────────────────────── */
/* ──────────────────────────────────────────────
   DENO AI CONFIG — lewat backend Flask (app.py)
   Browser TIDAK menyimpan/menanyakan API key apa pun.
   API key (Groq) sudah diisi otomatis di file .env
   yang dibaca oleh app.py, jadi begitu "python app.py"
   dijalankan, fitur Tanya AI langsung aktif.

   Jika app.py dijalankan di komputer/port lain, ubah
   AI_API_URL di bawah ini sesuai alamatnya.
────────────────────────────────────────────── */
...
async function cekStatusBackendAI() {
  ...
  try {
    ...
    if (res.ok && data.provider_ready) {
      ...
      tambahBubbleAI(
        'assistant',
        '⚠️ <strong>Backend DENO AI belum siap.</strong><br><br>' +
    ...
    tambahBubbleAI(
      'assistant',
      '⚠️ <strong>Tidak dapat terhubung ke backend DENO AI.</strong><br><br>' +
  ...
  try {
    ...
    if (!response.ok) {
      const pesanError = data.error
        ? `⚠️ ${data.error}`
        : '⚠️ Maaf, DENO AI sedang tidak bisa diakses. Pastikan backend (python app.py) sedang berjalan.';
    ...
    tambahBubbleAI(
      'assistant',
      '⚠️ Tidak dapat terhubung ke backend DENO AI.<br><br>' +
