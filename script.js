...
/**
 * Kirim payload rekap (tipe 'evaluasi' atau 'progresKB') LANGSUNG ke Web
 * App Google Apps Script (Code.gs) — dipakai karena hosting statis (mis.
 * GitHub Pages) tidak bisa menjalankan app.py sama sekali. Beda dari versi
 * lama (fetch dengan mode:'no-cors'), fungsi ini benar-benar MEMBACA
 * balasannya — kalau Apps Script gagal (deployment nonaktif, error skrip,
 * dll), error itu di-throw supaya bisa ditampilkan ke pengguna, bukan
 * disembunyikan di balik pesan "berhasil" yang keliru.
 */
async function kirimKeRekap(payload) {
  ...
  try {
    res = await fetch(GOOGLE_SHEETS_WEBAPP_URL, {
      ...
      // tidak mengirim CORS preflight — lihat catatan di GOOGLE_SHEETS_WEBAPP_URL.
      headers: { 'Content-Type': 'text/plain;charset=utf-8' },
      body: JSON.stringify(payload),
    });
  } catch (networkErr) {
    // fetch() sendiri gagal (offline, atau CORS/deployment Apps Script
    // bermasalah sampai browser tidak bisa membaca balasannya sama sekali) —
    // beda dari server membalas dengan status error.
    throw new Error(
      'Tidak bisa menghubungi Google Apps Script. Cek koneksi internet, dan ' +
      'pastikan URL Web App di script.js (GOOGLE_SHEETS_WEBAPP_URL) memakai ' +
      '"/exec" (bukan "/dev") serta akses deployment-nya diset ke "Anyone".'
    );
  }
  ...
  if (!res.ok || data.status !== 'ok') {
    if (res.status === 404 || res.status === 405) {
      throw new Error(
        `URL Google Apps Script tidak valid atau salah metode (HTTP ${res.status}). ` +
        'Cek lagi GOOGLE_SHEETS_WEBAPP_URL di script.js — pastikan diakhiri "/exec" ' +
        'dan deployment-nya masih aktif (Deploy > Manage deployments).'
      );
    }
