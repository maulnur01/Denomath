...
/* ──────────────────────────────────────────────
   ...
   memanggil LANGSUNG URL Web App Google Apps Script (Code.gs) — bukan
   lewat /api/rekap. Lihat kirimRekapLangsung() di bawah untuk kenapa ini
   sekarang memakai mode 'no-cors' (perbaikan penting, baca komentarnya).
   ...
   ⚠️ NILAI DI BAWAH INI MASIH URL PLACEHOLDER/CONTOH — belum tentu
   sudah menunjuk ke spreadsheet BARU di atas. WAJIB diganti manual
   dengan URL "/exec" hasil deploy-mu sendiri sebelum dipakai murid,
   kalau tidak rekap akan gagal terkirim (lihat health-check konsol
   browser untuk pesan errornya). */
 ...
 *
 * Ada dua jalur, dicoba berurutan supaya OTOMATIS menyesuaikan ke mana
 * pun DENOMATH dibuka, TANPA perlu ubah kode:
 *
 * 1. kirimRekapViaProxy() — lewat backend Flask sendiri (REKAP_URL,
 *    endpoint /api/rekap di app.py yang meneruskan ke Code.gs).
 *    Dipakai kalau app.py memang sedang berjalan (mis. dibuka lewat
 *    http://localhost:5000/ atau server sendiri yang menjalankan Python).
 *
 * 2. kirimRekapLangsung() — kalau jalur 1 gagal karena /api/rekap TIDAK
 *    ADA sama sekali (HTTP 404 — tandanya hosting statis seperti GitHub
 *    Pages, yang tidak menjalankan app.py), browser memanggil Web App
 *    Google Apps Script (Code.gs) LANGSUNG. Inilah jalur yang membuat
 *    rekap tetap REALTIME walau di-hosting statis di GitHub — lihat
 *    komentar di kirimRekapLangsung() untuk detail perbaikan CORS-nya.
 */
 ...
 *
 * PERBAIKAN PENTING dari versi sebelumnya: fetch() sebelumnya dipanggil
 * dengan mode default ('cors'), yang mewajibkan browser BISA MEMBACA
 * balasan Apps Script untuk menganggap request sukses. Masalahnya, Apps
 * Script Web App TIDAK PERNAH mengirim header "Access-Control-Allow-Origin"
 * — jadi walau data-nya SUDAH tersimpan di Spreadsheet dengan benar di
 * sisi server Google, browser tetap MEMBLOKIR pembacaan balasannya dan
 * fetch() melempar error jaringan (TypeError: Failed to fetch). Inilah
 * sebab utama rekap terasa "tidak realtime"/selalu gagal saat DENOMATH
 * di-hosting statis di GitHub Pages, PADAHAL request-nya sendiri sukses.
 *
 * Solusinya: pakai mode: 'no-cors'. Ini membuat browser tetap MENGIRIM
 * request apa adanya (Code.gs tetap jalan & tetap menulis ke Spreadsheet
 * seperti biasa) hanya saja balasannya jadi "opaque" (tidak bisa dibaca
 * isi/status HTTP-nya sama sekali — ini batasan keamanan browser, bukan
 * sesuatu yang bisa "diperbaiki" lebih lanjut dari sisi kode). Karena
 * response TIDAK BISA dibaca, di sini SENGAJA dianggap sukses selama
 * fetch()-nya sendiri tidak melempar error jaringan (mis. putus koneksi/
 * DNS gagal) — konsekuensinya, kalau URL Web App salah/deployment mati,
 * DENOMATH tidak akan tahu dari sini (makanya antrean kirim-ulang di
 * bawah tetap dipertahankan sebagai jaring pengaman, dan selalu cek
 * langsung ke Spreadsheet untuk memastikan data benar-benar masuk).
 */
