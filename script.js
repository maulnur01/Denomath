...
/* ──────────────────────────────────────────────
   ...
   ke Spreadsheet BARU secara realtime walau tanpa app.py, browser di sini
   memanggil LANGSUNG URL Web App Google Apps Script (Code.gs) sebagai
   request GET (bukan lewat /api/rekap, dan bukan POST — lihat komentar
   lengkap di kirimRekapLangsung() untuk alasannya).

   Spreadsheet tujuan sekarang (rekapan BARU):
   ...
   Deploy > New deployment (atau Manage deployments) dari Code.gs yang
   ditempel di DALAM spreadsheet BARU di atas (Who has access: Anyone).
   Lihat langkah lengkapnya di komentar atas Code.gs. URL "/dev" hanya
   bisa diakses saat KAMU sendiri login ke akun Google pemilik script,
   jadi TIDAK akan berfungsi untuk murid lain. */
const GOOGLE_SHEETS_WEBAPP_URL =
  'https://script.google.com/macros/s/AKfycbx9yZE-ut2h3MZsli8lna70T4iDg_fChs2WOTzZ84uPT0i4xzbWPGJB58-2JG2yq9ikow/exec';
 ...
 * Ada dua jalur:
 *
 * 1. kirimRekapViaProxy() — lewat backend Flask sendiri (REKAP_URL,
 *    endpoint /api/rekap di app.py yang meneruskan ke Code.gs). Dipakai
 *    kalau app.py memang sedang berjalan (mis. dibuka lewat
 *    http://localhost:5000/ atau server sendiri yang menjalankan Python).
 *
 * 2. kirimRekapLangsung() — dicoba SETIAP KALI jalur 1 gagal, apa pun
 *    alasannya (bukan cuma saat /api/rekap membalas persis HTTP 404).
 *
 *    PERBAIKAN PENTING dari versi sebelumnya: sebelumnya jalur 2 CUMA
 *    dicoba kalau proxy membalas status persis 404. Ini ternyata rapuh —
 *    penyedia hosting statis beda-beda caranya membalas request POST ke
 *    path yang tidak ada (GitHub Pages/CDN sejenis bisa saja membalas
 *    405 Method Not Allowed, mengembalikan halaman lain dengan status
 *    200, atau bentuk lain yang BUKAN 404 polos) — akibatnya jalur
 *    cadangan tidak pernah tercoba sama sekali walau app.py memang tidak
 *    berjalan, dan rekap terlihat "tidak pernah masuk" ke Spreadsheet.
 *    Sekarang, kegagalan proxy APA PUN (404, 405, error jaringan, JSON
 *    tidak valid, dst) langsung memicu percobaan jalur langsung sebagai
 *    upaya kedua — jauh lebih tahan terhadap perbedaan perilaku tiap
 *    penyedia hosting statis.
 */
async function kirimKeRekap(payload) {
  try {
    ...
    try {
      ...
      // Proxy Flask maupun panggilan langsung ke Apps Script dua-duanya
      // gagal — lempar error dari proxy (biasanya pesannya lebih spesifik
      // soal konfigurasi server) supaya guru/murid tahu apa yang harus
      // dicek duluan.
      throw proxyErr;
    }
  }
...
async function kirimRekapViaProxy(payload) {
  ...
  // Catatan: dulu di sini ada pengecualian khusus untuk res.status === 404
  // (dianggap "hosting statis, /api/rekap memang tidak ada"). Sekarang
  // TIDAK perlu lagi — kirimKeRekap() di atas mencoba kirimRekapLangsung()
  // untuk kegagalan proxy APA PUN, jadi cukup lempar error apa adanya di
  // sini untuk semua kasus non-sukses.
...
/**
 * Jalur cadangan (hosting statis, TANPA app.py sama sekali — mis. GitHub
 * Pages): panggil Web App Google Apps Script LANGSUNG dari browser.
 *
 * PERBAIKAN PENTING dari versi sebelumnya: sebelumnya jalur ini mengirim
 * data lewat POST dengan body JSON (pakai mode: 'no-cors' supaya request-
 * nya sendiri tidak diblokir CORS). Masalahnya, Web App Google Apps
 * Script sering membalas dengan REDIRECT internal (dari domain
 * script.google.com ke script.googleusercontent.com) sebelum benar-benar
 * menjalankan doPost — dan browser MENGUBAH method jadi GET & MEMBUANG
 * body saat otomatis mengikuti redirect semacam itu (ini perilaku standar
 * fetch/browser untuk redirect 302/303, bukan bug). Akibatnya doPost bisa
 * menerima body KOSONG walau request "kelihatannya" terkirim sukses dari
 * sisi browser (karena mode no-cors membuat kita tidak bisa membaca
 * balasannya sama sekali untuk tahu ini terjadi) — inilah kemungkinan
 * besar penyebab rekap masih belum masuk ke Spreadsheet walau URL Web App
 * sudah benar.
 *
 * Solusinya: kirim sebagai GET dengan data di query string (memanggil
 * doGet di Code.gs, bukan doPost). GET tidak punya body sama sekali,
 * jadi tidak ada apa pun yang bisa "hilang" walau requestnya di-redirect.
 * Tetap pakai mode: 'no-cors' karena balasannya tetap tidak bisa dibaca
 * (Apps Script tidak pernah mengirim header CORS), tapi itu tidak masalah
 * di sini — yang penting datanya sampai & tertulis ke Spreadsheet.
 */
async function kirimRekapLangsung(payload) {
  // Hanya field yang benar-benar dibaca oleh handleRequest/simpanEvaluasi/
  // updateProgresKB di Code.gs yang disertakan, supaya URL tetap ringkas
  // (field lain seperti `detail`/`kb1_ga_link` dst tidak dipakai di sana).
  const FIELD_DIPAKAI = [
    'tipe', 'timestamp', 'nama', 'kelas', 'absen', 'nilai',
    'loginTipe', 'anggota', 'kb1_skor_total', 'kb2_skor_total',
  ];
  const params = new URLSearchParams();
  FIELD_DIPAKAI.forEach((key) => {
    const val = payload[key];
    if (val !== undefined && val !== null && val !== '') {
      params.set(key, String(val));
    }
  });
  const url = `${GOOGLE_SHEETS_WEBAPP_URL}?${params.toString()}`;

  try {
    await fetch(url, { method: 'GET', mode: 'no-cors' });
  } catch (networkErr) {
    throw new Error(
      'Tidak bisa menghubungi Google Apps Script dari browser (kemungkinan ' +
      'tidak ada koneksi internet, atau URL GOOGLE_SHEETS_WEBAPP_URL di ' +
      'script.js belum diganti dengan URL "/exec" hasil deploy Code.gs ' +
      'ke spreadsheet rekap yang baru).'
    );
  }
  // Response bersifat "opaque" (mode no-cors) — tidak ada status/JSON yang
  // bisa dibaca, jadi di sini dianggap terkirim selama tidak ada error di atas.
  return { status: 'ok', opaque: true };
}
...
/* ──────────────────────────────────────────────
   MASKOT TIKUS BERLARI — dekorasi mengambang yang
   tampil di semua halaman (di luar #page-xxx, jadi
   tidak ikut hilang saat showPage() ganti halaman).
   - Bubble teks otomatis redup setelah beberapa detik
     supaya tidak mengganggu, tapi muncul lagi saat
     mascot disentuh/di-hover.
   - Tombol (x) menyembunyikan maskot; tombol emoji 🐭
     kecil di pojok akan muncul untuk memunculkannya lagi.
────────────────────────────────────────────── */
function toggleMouseMascot(tampilkan) {
  const mascot = document.getElementById('mouseMascot');
  const reopenBtn = document.getElementById('mouseMascotReopen');
  if (!mascot || !reopenBtn) return;
  if (tampilkan) {
...
document.addEventListener('DOMContentLoaded', function () {
  const mascot = document.getElementById('mouseMascot');
  if (!mascot) return;
  // Bubble sapaan otomatis redup setelah 5 detik biar tidak menutupi konten,
  // lalu muncul lagi tiap kali mascot disentuh/dihover.
  setTimeout(() => mascot.classList.add('bubble-idle'), 5000);
  mascot.addEventListener('mouseenter', () => mascot.classList.remove('bubble-idle'));
  mascot.addEventListener('touchstart', () => mascot.classList.remove('bubble-idle'), { passive: true });
});  