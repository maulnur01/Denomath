...
# berubah). Nilai di bawah ini hanya fallback dari URL yang sebelumnya
# ditulis langsung di script.js.
# ---------------------------------------------------------------------------
GOOGLE_SHEETS_URL = (os.environ.get("GOOGLE_SHEETS_URL") or
    "https://script.google.com/macros/s/AKfycbyohfwRkSsNpk7zDupJFPj-C9w-a7Q5LOzqITtHq0M/dev"
).strip()

if not GOOGLE_SHEETS_URL:
    logger.warning(
        "⚠️  GOOGLE_SHEETS_URL belum diisi di file .env. "
        "Endpoint /api/rekap (rekap nilai ke Spreadsheet) akan mengembalikan error 503."
    )
...
ALLOWED_STATIC_FILES = {"index.html", "style.css", "script.js", "logo.png"}
...
def rekap():
    ...
    if not GOOGLE_SHEETS_URL:
        return jsonify({
            "status": "error",
            "message": "GOOGLE_SHEETS_URL belum diisi di file .env pada server.",
        }), 503
...
def health():
    return jsonify({
        ...
        "model": GROQ_MODEL,
        "sheets_rekap_ready": bool(GOOGLE_SHEETS_URL),
