...
# Spreadsheet ini belum punya Apps Script Web App terpasang sampai Code.gs
# ditempel & di-deploy dari DALAM spreadsheet tsb (lihat instruksi lengkap
# di komentar atas Code.gs). Setelah deploy, ganti fallback di bawah (atau,
# lebih aman, isi lewat GOOGLE_SHEETS_URL di file .env) dengan URL "/exec"
# hasil deploy tersebut.
#
...
GOOGLE_SHEETS_URL = (os.environ.get("GOOGLE_SHEETS_URL") or
    "GANTI_DENGAN_URL_WEB_APP_HASIL_DEPLOY_CODE_GS_DI_SPREADSHEET_BARU"
).strip()

# Fallback di atas cuma placeholder teks (bukan URL valid) sampai Code.gs
# benar-benar di-deploy ke spreadsheet baru dan URL "/exec" hasilnya
# ditempel menggantikan placeholder ini (atau diisi lewat .env).
